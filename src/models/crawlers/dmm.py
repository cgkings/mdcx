...
def get_trailer(htmlcode, real_url):
    ...
    vr_cid = re.findall(
        r"https://www.dmm.co.jp/digital/-/vr-sample-player/=/cid=([^/]+)", htmlcode
    )
    if normal_cid:
        cid = normal_cid[0]
        ...
        cid = vr_cid[0]
        temp_url = "https://cc3001.dmm.co.jp/vrsample/{0}/{1}/{2}/{2}vrlite.mp4".format(
            cid[:1], cid[:3], cid
        )
...
def get_real_url(html, number, number2, file_path):
    ...
    for each in url_list:
        if (number_pre.search(each) and number_end.search(each)) or number_mid.search(
            each
        ):
        ...
        if title_list and url_list:
            ...
            for i in range(len(url_list)):
                ...
                if temp_title in full_title:
                    ...
                    cid = re.findall(
                        r"(cid|content)=.*?([a-z]{3,})0*(\d{3,}[a-z]*)", temp_url
                    )
                    if cid:
...
def get_tv_jp_data(real_url):
    ...
    if result and response.get("data"):
        ...
        try:
            trailer_url = api_data["sampleMovie"]["url"].replace(
                "hlsvideo", "litevideo"
            )
...
def main(number, appoint_url="", log_info="", req_web="", language="jp", file_path=""):
    ...
    if not appoint_url:
        real_url = (
            "https://www.dmm.co.jp/search/=/searchstr=%s/sort=ranking/" % number_00
        )  # 带00
    ...
    try:
            ...
            if not appoint_url:
                ...
                if not real_url:
                    ...
                    if number_no_00 != number_00:
                        real_url = (
                            "https://www.dmm.co.jp/search/=/searchstr=%s/sort=ranking/"
                            % number_no_00
                        )  # 不带00，旧作 snis-027
                        ...
                        if not result:  # 请求失败
                            ...
                            raise Exception(debug_info)
                        html = etree.fromstring(htmlcode, etree.HTMLParser())
                        real_url, number = get_real_url(
                            html, number, number_no_00, file_path
                        )
                        if not real_url:
                            debug_info = "搜索结果: 未匹配到番号！"
                            log_info += web_info + debug_info
                ...
                if not real_url:
                    real_url = (
                        "https://www.dmm.com/search/=/searchstr=%s/sort=ranking/"
                        % number_no_00
                    )
                    debug_info = "再次搜索地址: %s " % real_url
                    log_info += web_info + debug_info
                    ...
                    real_url.find("?i3_ref=search&i3_ord") != -1
                ):  # 去除url中无用的后缀
        ...
        if not real_url or "tv.dmm.com" in real_url:
            if not real_url:
                ...
                debug_info = "番号地址: %s " % real_url
                number_00 = (
                    re.findall(r"season=([^&]+)", real_url)[0]
                    if "season=" in real_url
                    else number_00
                )
            log_info += web_info + debug_info
            (
    ...
    js = json.dumps(
        dic, ensure_ascii=False, sort_keys=False, indent=4, separators=(",", ": ")
    )  # .encode('UTF-8')
    return js
